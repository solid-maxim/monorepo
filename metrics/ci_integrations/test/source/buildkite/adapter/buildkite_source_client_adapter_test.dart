...
void main() {
  group("BuildkiteSourceClientAdapter", () {
    ...
    test(
      ".fetchBuilds() maps fetched builds' api url to the empty string if the url is null",
      () async {
        ...
        final result = await adapter.fetchBuilds(pipelineSlug);
        final apiUrl = result.first.apiUrl;

        expect(apiUrl, equals(''));
      },
    );
    ...
    test(
      ".fetchBuildsAfter() maps fetched builds' api url to the empty string if the url is null",
      () async {
        const build = BuildkiteBuild(
          number: 2,
          blocked: false,
